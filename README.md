...
- **Functional HTTP interceptor for errors.** A single `errorInterceptor`
  ...
  registered through `provideHttpClient(withInterceptors([...]))`.
...
src/app/
  core/
    interceptors/   # functional HTTP interceptors (error handling)
...
- **Interceptor tests** (`error.interceptor.spec.ts`) verify error handling behavior in isolation
...
- **Client-side-only activity feed.** `ActivityStore` only reflects mutations made in the current
  ...
  endpoint exists.
- **No offline/optimistic UI.** Task mutations wait for the HTTP round-trip and an explicit
  `resource.reload()` before the UI reflects the change; there's no optimistic update or offline
  queueing.
- **No E2E test suite.** Only unit/component tests via Vitest exist today; Angular CLI does not
  ship an e2e framework by default and one hasn't been added yet (candidate: Playwright).
- **No caching/retry HTTP interceptors.** Only the error interceptor is currently registered;
  request caching and retry-on-transient-failure were considered but not implemented in this pass.
- **Potential future improvements**: add Playwright e2e coverage for the kanban drag-and-drop flow,
  add a caching interceptor for read-heavy endpoints (statistics, users), add optimistic updates to
  `TaskStore` mutation methods, and introduce route guards once/if authentication is added.
